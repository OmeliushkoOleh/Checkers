...
const Fild: React.FC<any> = () => {
  ...
  const startNewGame = ()=>{
    ...
    for(let x = 1;x <=8; x++){
      for(let y = 1;y <=8; y++){
        ...
        let comp = <Cell   setStep={setStep} canDrop={false} setArr={setArr} x={x} y={y} color={color} key={Math.random()*100} checker={checker}></Cell>
  ...
  return <div className={styles.fild}>
     ...
     <div className="player"> 
      <button onClick={showStep} >Посмотреть Ход №</button>
      <button onClick={showPrevStep}>&#5176;</button>
      <input type="number" className="input" min={1} ref={inputref}></input> 
      <button onClick={showNextStep}>&#5171;</button>
      <button></button>
     </div>
